...
if os.path.exists('options.txt') and os.path.exists('user'):
    f = open('options.txt','r') # Espero que vc não se ache hacker por saber mexer com esse arquivo
    data = f.read()
    login = int(data[0])
    cpf_api = int(data[1])
    ip_api = int(data[2])
...
if login == 1:
    ...
    if input("PASSWORD:  ").strip() == snh:
        ...
        print("{C}[{R}ERROR{C}] Revise a senha, ela parece estar incorreta")
        if anim == 1:
            time.sleep(1)
        exit()
    print("\n ")
...
while(Sair == False):
    ...
    print(f"{C}{G}[3]{C} EDU-SITE-INFOGA")
